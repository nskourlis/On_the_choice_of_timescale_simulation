...
cd  "$N\5b.Performance_results"
	...
	foreach X in CIF1_f${female}_at1 CIF1_f${female}_at2 CIF1_f${female}_at3 CIF1_f${female}_at4 CIF1_f${female}_at5 CIF1_f${female}_at6 CIF1_f${female}_at7 CIF1_f${female}_at8 CIF1_f${female}_at9 CIF1_f${female}_at10   ///
	 ...
	 summ `X' if givenage==`i'
	 local mean_est_`X'_bias_`i'=`r(mean)'
	}
	}
...
postfile comparison     CIF1_f${female}_at1_t1t2 CIF1_f${female}_at2_t1t2 CIF1_f${female}_at3_t1t2 CIF1_f${female}_at4_t1t2 CIF1_f${female}_at5_t1t2 CIF1_f${female}_at6_t1t2 ///
			            ...
			            using ///
			"$N\5b.Performance_results\compare_attained\scen_`scen'_compare_attained_f`female'.dta", replace
...
local bias_`k'_t1t1= `r_b_`k'_bias_`i'_t1t2'	

local MCer_`k'_t1t2= `MCer_`k'_`i'_t1t2'                                   //Monte carlo errors in approach a-Attained

local MCer_`k'_t1t1= `MCer_`k'_`i'_t1t2'	

local rb_MCer_`k'_t1t2= `rb_MCer_`k'_`i'_t1t2'                                   //Monte carlo errors in approach a-Attained

local rb_MCer_`k'_t1t1= `rb_MCer_`k'_`i'_t1t2'
	
local bias_dif_`k'= .

local rel_bias_`k'_t1t2= (`b_`k'_bias_`i'_t1t2'/${`k'_true} ) *100    //Relative bias in approach a-Attained

local rel_bias_`k'_t1t1= (`b_`k'_bias_`i'_t1t2'/${`k'_true} ) *100

local EmpSE_`k'_t1t1= `EmpSE_`k'_`i'_t1t2'                            //Empirical st.errors in approach a-Attained

local EmpSE_`k'_t1t2= `EmpSE_`k'_`i'_t1t2'

local Rel_prec_`k'=.

local Coverage_`k'_t1t2= `coverage_`k'_`i'_t1t2'                           //Coverage of approach a-Attained

local Coverage_`k'_t1t1= `coverage_`k'_`i'_t1t2'
...
local convergence_t1t1=`convergence_t1t2'                                 
...
foreach t in hr_gender_at1 hr_gender_at2 hr_gender_at3 hr_gender_at4 hr_gender_at5 /// 
			 hr_gender_at6 hr_gender_at7 hr_gender_at8 hr_gender_at9 hr_gender_at10 { 
				 
	local  bias_`t'_t1t2=	`b_`t'_bias_`i'_t1t2'
	local  bias_`t'_t1t1=	`b_`t'_bias_`i'_t1t2'

	local  `t'_t1t2= 	``t'_`i'_t1t2' 
	local  `t'_t1t1= 	``t'_`i'_t1t2'
...
use "$N\5b.Performance_results\compare_attained\scen_`scen'_compare_attained_f1.dta", replace

 merge 1:1 _n using "$N\5b.Performance_results\compare_attained\scen_`scen'_compare_attained_f0.dta"
save "$N\5b.Performance_results\compare_attained\scen_`scen'_compare_attained.dta", replace

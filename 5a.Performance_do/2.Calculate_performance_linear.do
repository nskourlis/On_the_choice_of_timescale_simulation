...
cd  "$N\5b.Performance_results"
	...
	use  "$N\4b.Analyze_results\Approach_b_linear/scenario`scen'_linear.dta", replace
...
// Derive the bias, convergence, relative bias and empirical standard errors 
	...
	local convergence_t1t2= (`r(N)'/1000)*100

	}
	}
	
		//Derive MCerrors and EmpSE
...
postfile comparison     CIF1_f${female}_at1_t1t2 CIF1_f${female}_at2_t1t2 CIF1_f${female}_at3_t1t2 CIF1_f${female}_at4_t1t2 CIF1_f${female}_at5_t1t2 CIF1_f${female}_at6_t1t2 ///
			            ...
			            using ///
			"$N\5b.Performance_results\compare_linear\scen_`scen'_compare_linear_f`female'.dta", replace
...
local bias_`k'_t1t1= `r_b_`k'_bias_`i'_t1t1'   //Bias in approach b-Linear

local MCer_`k'_t1t2= `MCer_`k'_`i'_t1t2'   //Monte carlo errors in approach a-Attained

local MCer_`k'_t1t1= `MCer_`k'_`i'_t1t1'	 //Monte carlo errors in approach b-Linear


local rb_MCer_`k'_t1t2= `rb_MCer_`k'_`i'_t1t2'                                   //Monte carlo errors in approach a-Attained
...
local rel_bias_`k'_t1t1= (`b_`k'_bias_`i'_t1t1'/${`k'_true} ) *100 //Relative bias in approach b-Linear

local EmpSE_`k'_t1t1= `EmpSE_`k'_`i'_t1t1'           //Empirical st.errors in approach a-Attained

local EmpSE_`k'_t1t2= `EmpSE_`k'_`i'_t1t2'        //Empirical st.errors in approach b-Linear

local Rel_prec_`k'=100* ((`EmpSE_`k'_`i'_t1t2'/`EmpSE_`k'_`i'_t1t1')^2-1)   //Relative precision approach b-Linear

local Coverage_`k'_t1t2= `coverage_`k'_`i'_t1t2'      //Coverage of approach a-Attained

local Coverage_`k'_t1t1= `coverage_`k'_`i'_t1t1'        //Coverage of approach b-Linear
...
local convergence_t1t1= `convergence_t1t1'                    //Convergence of approach b-Linear
...
use "$N\5b.Performance_results\compare_linear\scen_`scen'_compare_linear_f1.dta", replace

 merge 1:1 _n using "$N\5b.Performance_results\compare_linear\scen_`scen'_compare_linear_f0.dta"
save "$N\5b.Performance_results\compare_linear\scen_`scen'_compare_linear.dta", replace
